//
//  schurNumberPermutationImproved.c
//  schurNumberSymmetric
//
//  Created by rubis on 09/06/2019.
//  Copyright © 2019 rubis. All rights reserved.
//

#include "schurNumberSymmetricHeader.h"

size_t schurNumberOrder2Unities(unsigned long n, unsigned long *group) {
    /*Cette fonction trouve toutes les inversibles d'ordre 2 de Z/(n+1)Z et les conserve dans group.
...
unsigned long schurNumberPermutationImproved(unsigned long n, partition_t *partitionstruc, unsigned long depth,  char completesearch) {
    /*
     Cette fonction trouve des partitions sans-sommes de Z/(n+1)Z à p huches.
     ...
     Cet algorithme repose sur l'observation que toute partition sans-somme de Z/(n+1)Z est stable par toutes les unités d'ordre 2. Par conséquent, chaque entier m+1 est adjoint avec tous les autres entiers conjugués, modulo le sous-groupe multiplicatif engendré par les unités d'ordre 2.
    ...
    mp_limb_t *remaining;   // Entier dont le bit m indique si m+1 a été placé
    ...
    mp_limb_t mask;
    mp_limb_t mask2;
    unsigned long *invertibles;
    size_t count_invertibles;
    ...
    remaining = calloc(limballoc, sizeof(mp_limb_t));
    ...
    /*Calcul des inversibles d'ordre 2*/
    count_invertibles = schurNumberOrder2Unities(n, invertibles);
