...
int main(int argc, const char * argv[]) {
    ...
    schurNumberActionAlloc(&action_s, p+1, actionfunc);
    ...
    if (0 < argc2) {
        ...
        partition_s.p = p;
        partition_s.n = 0;
        ADD_POINT(partition_s.partition[p-1], 0);
        ADD_POINT(partition_s.partitioninvert[p-1], limballoc * mp_bits_per_limb - 1);
    }
    ...
    switch (method) {
        case '1':
            methodfunc = schurNumberPuncturedInterval;
            break;
